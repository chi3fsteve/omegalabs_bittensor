...
def get_llm_prompt(query: str) -> str:
    return f"Take the given query `{query}` and augment it to be more detailed. For example, add specific names, types, embellishments, richness. The goal is to search for the most obscure videos using this prompt so realy try combining the topic with some other thing thats related. Do not make it longer than 12 words."
