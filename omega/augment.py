...
def get_llm_prompt(query: str) -> str:
    return f"Take the given query `{query}` and combine it with a topic that is related, then augment it to be more detailed and unique. For example, add specific names, types, embellishments, richness. The goal is to search for the most obscure videos using this prompt. Do not make it longer than 12 words."
