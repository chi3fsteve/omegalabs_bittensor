...
def get_llm_prompt(query: str) -> str:
    return f"Take the given query `{query}` and augment it to be more detailed and unique. For example, add specific names, types, embellishments, richness. The goal is to search for the most obscure videos using this prompt, so try combining the topic with some other thing that is related but not necessarily obvious. Do not make it longer than 12 words."
